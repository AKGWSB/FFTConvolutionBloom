    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                //return float4(_MainTex.Sample(sampler_LinearRepeat, i.uv - 0.5).rgb, 0.0);
                if(bUseLuminance)
                {
                    float lum = Luma(_MainTex.Sample(sampler_LinearClamp, xy).rgb);
                    return float4(float3(lum, lum, lum) * kernelScale, 0.0);
                }
                return float4(_MainTex.Sample(sampler_LinearClamp, xy).rgb * kernelScale, 0.0);
            }
            ENDHLSL
        }
    }
